...
conv1 = tf.nn.conv2d(X, W1, strides=[1,2,2,1], padding="SAME",use_cudnn_on_gpu=True)
...
conv2 = tf.nn.conv2d(conv1, W2, strides=[1,1,1,1], padding='SAME',use_cudnn_on_gpu=True)
...
conv3 = tf.nn.conv2d(max_pool2, W3, strides=[1,2,2,1], padding='SAME',use_cudnn_on_gpu=True)
...
conv4 = tf.nn.conv2d(conv3, W4, strides=[1,1,1,1], padding='SAME',use_cudnn_on_gpu=True)
b4 = tf.Variable(tf.constant(0.0, shape=[128], dtype = tf.float32), trainable=True, name='b4')
conv4 = tf.nn.relu(tf.nn.bias_add(conv4, b4))
max_pool4 = tf.nn.max_pool(conv4, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')


#fully_connected1
flat = tf.reshape(max_pool4, [-1, 16*16*128])
W11 = tf.get_variable("w11", shape = [16*16*128, 512], dtype = tf.float32, initializer=tf.contrib.layers.xavier_initializer_conv2d())
